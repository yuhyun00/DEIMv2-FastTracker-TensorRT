...
void STrack::activate(fast_kalman::KalmanFilter &kalman_filter, int frame_id)
{
...
void STrack::re_activate(STrack &new_track, int frame_id, bool new_id)
{
	...
	mean_history.push_back(this->mean);
	cov_history.push_back(this->covariance);
	if (mean_history.size() > max_history_len)
		mean_history.pop_front();
	if (cov_history.size() > max_history_len)
		cov_history.pop_front();
	//
...
void STrack::multi_predict(vector<STrack*> &stracks, fast_kalman::KalmanFilter &kalman_filter)
{
