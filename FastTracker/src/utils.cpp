...
vector<STrack*> FastTracker::joint_stracks(vector<STrack*> &tlista, vector<STrack> &tlistb)
{
	map<int, int> exists;
	vector<STrack*> res;
...
vector<STrack> FastTracker::sub_stracks(vector<STrack> &tlista, vector<STrack> &tlistb)
{
...
void FastTracker::remove_duplicate_stracks(vector<STrack> &resa, vector<STrack> &resb, vector<STrack> &stracksa, vector<STrack> &stracksb)
{
...
void FastTracker::linear_assignment(vector<vector<float> > &cost_matrix, int cost_matrix_size, int cost_matrix_size_size, float thresh,
...
vector<vector<float> > FastTracker::ious(vector<vector<float> > &atlbrs, vector<vector<float> > &btlbrs)
{
...
vector<vector<float> > FastTracker::iou_distance(vector<STrack*> &atracks, vector<STrack> &btracks, int &dist_size, int &dist_size_size)
{
...
vector<vector<float> > FastTracker::iou_distance(vector<STrack> &atracks, vector<STrack> &btracks)
{
	vector<vector<float> > atlbrs, btlbrs;
	for (int i = 0; i < atracks.size(); i++)
	{
...
double FastTracker::lapjv(const vector<vector<float> > &cost, vector<int> &rowsol, vector<int> &colsol,
